...
function GalleryCard({ work }: { work: GalleryItem }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-300 transform hover:-translate-y-1 group flex flex-col justify-between">
      {/* Imagen del corte */}
      <div className="relative w-full bg-slate-50 overflow-hidden flex items-center justify-center text-slate-300 group-hover:text-slate-400 transition-colors">
        {currentMedia ? (
          currentMedia.toLowerCase().match(/\.(mp4|webm)$/i) ? (
            <video src={currentMedia} autoPlay loop muted playsInline controls={false} className="w-full h-auto block rounded-t-xl group-hover:scale-105 transition-all duration-500" />
          ) : (
            // eslint-disable-next-line @next/next/no-img-element
            <img
              src={currentMedia}
              alt={work.title}
              className="w-full h-auto block rounded-t-xl group-hover:scale-105 transition-all duration-500"
              loading="lazy"
              onError={(e) => {
